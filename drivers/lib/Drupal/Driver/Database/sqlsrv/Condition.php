    ...
    foreach ($this->conditions as &$condition) {
      if(isset($condition['operator'])) {
        if ($condition['operator'] == 'REGEXP') {
          $condition['field'] = 'REGEX(' . $connection->escapeField($condition['field']) . ', \'' . $condition['value'] . '\') = 1';
          $condition['operator'] = NULL;
          $condition['value'] = [];
        } else if ($condition['operator'] == 'NOT REGEXP') {
          $condition['field'] = 'REGEX(' . $connection->escapeField($condition['field']) . ', \'' . $condition['value'] . '\') = 0';
          $condition['operator'] = NULL;
          $condition['value'] = [];
        } 
      }
